...
:guilabel:`Note:` Splunk can increase this limit on a customer's request. The customer is subject to overage charges.
...
What happens when the limit is hit?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Any log data that exceeds the limit in that bucket (hourly and minutely) will be dropped and not indexed.
...
:guilabel:`Note:` Thirty-two rules is a hard limit. Splunk cannot increase this limit at a customer's request.
