...
.. meta::
   :description: Splunk Infrastrututre Monitoring uses detectors, events, alerts, and notifications to keep you informed when certain criteria are met. When a detector condition is met, the detector generates an event, triggers an alert, and can send one or more notifications.

Splunk Infrastructure Monitoring uses :strong:`detectors`, :strong:`events`, :strong:`alerts`, and :strong:`notifications` to keep you informed when certain criteria are met. 
...
-  When a detector is triggered, it does the following:
   ...
   -  Triggers an alert, which can be viewed in a number of places throughout Splunk Infrastructure Monitoring.
